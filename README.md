...
- オリジナルゆっくり風 SVG アセット 5 体 x 9 表情。霊夢/魔理沙は赤リボン・黒魔女帽などの識別仕様に合わせて再作成
- 黒板教室、ニュース机、畳部屋、夜景、紙ノート、分析グリッド、歴史資料室、科学宇宙、テック研究室、ミステリー部屋、経済ボード、検証法廷の背景アセット
- 疑問、衝撃、スピード線、章切替、ハイライト、注意喚起、出典表示などの同梱エフェクトアセット
...
`asset` は `classroom-board`, `news-desk`, `tatami-room`, `night-city`, `paper-light`, `studio-grid`, `history-archive`, `science-space`, `tech-lab`, `mystery-room`, `economy-board`, `courtroom` を指定できます。
...
`effect` は `speed-lines`, `impact-burst`, `question-pop`, `chapter-wipe`, `highlight-ring`, `sparkle-trail`, `danger-stripe`, `source-note` を指定できます。プレビューとMP4書き出しの両方に反映されます。

## 二次創作素材方針

霊夢/魔理沙アセットは、公式ゲーム素材や他者の二次創作素材を抽出・トレスせず、同梱スクリプトで生成した独自SVGです。投稿時は必要に応じて東方Project二次創作である旨を明記してください。
