...
- AquesTalkPlayer integration is user-selected or detected from explicit safe locations. The app accepts only an `.exe` named `AquesTalkPlayer.exe`, stores the path in local user data, normalizes known preset aliases, and launches it through `Start-Process -Wait` with quoted arguments so WAV generation is complete before the renderer or exporter proceeds.
